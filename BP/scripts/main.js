...
Math.eval = function (expr, x) {
    return Parser.evaluate(expr.replaceAll("n", x));
}
let removTag = [];
let toAdd = [];

function matchCmd(cmd, sender, tag) {
    if (cmd[0] == "test")
        system.run(() => {
        ...
        system.run(() => {
            //world.sendMessage(cmd[1]);
            //sender.runCommand(cmd[1].slice(1, -1));
        ...
        if (cmd[1] == "ask") {
            world.sendMessage(sender.id)

            //if (world.scoreboard.getObjective(cmd[2]))
            toAdd.push([sender.id, cmd[2], Number(cmd[3])]);
            //else world.sendMessage("An error has occured!");
            ...
            world.sendMessage("pi")
            for (let entity of overworld.getPlayers()) {
                for (let tag of sender.getTags()) {
                    if (entity.id == tag) {
                        system.run(() => {
                            world.scoreboard.getObjective(toAdd[toAdd.indexOf([sender.id, cmd[2], Number(cmd[3])]) + 1][1]).addScore(entity, Number(cmd[4]));
    ...
    for (let entity of overworld.getEntities()) {
        for (let tag of entity.getTags()) {
            if (tag.substr(0, 1) == cmdPrefix)
                matchCmd(tag.substr(1, tag.length).match(/\|.*?\||\S+/g), entity, true);
        }
    }
...
world.beforeEvents.chatSend.subscribe((ed) => {
    if (ed.message.substr(0, 1) == cmdPrefix) {
        ...
        matchCmd(ed.message.substr(1, ed.message.length).match(/\|.*?\||\S+/g), ed.sender, false);
    }
});
