...
import { world, system, EntityComponentTypes, EntityHealableComponent } from "@minecraft/server";
...
function ifTrueTag(player, action) {
    system.run(() => {
        if (player[action] == true && !player.hasTag("?" + action) && player.hasTag("?detect?")) player.addTag("?" + action);
        else if (player[action] == false && player.hasTag("?" + action) && player.hasTag("?detect?")) player.removeTag("?" + action);
    });
}
//world.scoreboard.addObjective("?health");
...
system.runInterval(() => {
    ...
    for (let entity of overworld.getEntities()) {
        ...
        if (entity.hasTag("?detect?") && world.scoreboard.getObjective("?health").getScore(entity) != entity.getComponent(EntityComponentTypes.Health).currentValue)
            world.scoreboard.getObjective("?health").setScore(entity, entity.getComponent(EntityComponentTypes.Health).currentValue);
    ...
    for (let player of overworld.getPlayers()) {
        ...
        //action detect. Only for players with tag "?detect?"
